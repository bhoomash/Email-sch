...
import { EMAIL_QUEUE_NAME, EmailJobData, enqueueEmailJob } from './email.queue.js';
...
export const emailWorker = new Worker<EmailJobData>(
  ...
  async (job: Job<EmailJobData>) => {
    const { emailId, campaignId, senderId, userId } = job.data;
    logger.info({ jobId: job.id, emailId }, 'Processing email job');
    ...
    if (!rateLimitCheck.allowed && rateLimitCheck.nextHourStart) {
      ...
      // Re-enqueue BullMQ job for next hour
      await enqueueEmailJob(job.data, delayMs);
